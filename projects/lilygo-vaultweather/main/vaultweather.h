 ...
 * alone. Indices run 0=NEW, 2=FIRST QUARTER, 4=FULL, 6=LAST QUARTER.
 *
 * NORTHERN HEMISPHERE ORIENTATION: waxing phases are lit on the RIGHT limb.
 * This is wrong below the equator, which is a deliberate limitation for a
 * device configured in Evansville, Indiana -- noted so it is a known bound
 * rather than a latent bug. */
#define WX_MOON_PHASES 8

extern const lv_image_dsc_t wx_moon_frames[WX_MOON_PHASES];

/* A cloud with the moon behind it, for PARTLY at night. Phase-independent:
 * the cloud covers most of the disc, so eight variants would be invisible
 * work. */
extern const lv_image_dsc_t wx_moon_cloud_frame;

int         wx_moon_phase(time_t t);        /* 0..WX_MOON_PHASES-1 */
const char *wx_moon_phase_name(int phase);  /* <=12 chars, uppercase */
