...
static void report_build(void)
{
    ...
    rule();
    printf("BOOT\n");
    /* Full esp_reset_reason_t, transcribed from esp_system.h rather than
       remembered. An earlier legend listed only seven values and omitted 11,
       which a native-USB board produces on EVERY flash -- so the single most
       common reason on this hardware printed as an unexplained number. */
    static const char *const RESET_REASON[] = {
        "unknown", "power-on", "external pin", "software (esp_restart)",
        "PANIC / exception", "interrupt watchdog", "task watchdog",
        "other watchdog", "deep-sleep wake", "BROWNOUT (power, not code)",
        "SDIO", "USB peripheral", "JTAG", "eFuse error",
        "power glitch", "CPU lockup (double exception)",
    };
    int rr = (int) esp_reset_reason();
    const char *name = (rr >= 0 && rr < (int)(sizeof(RESET_REASON) / sizeof(RESET_REASON[0])))
                       ? RESET_REASON[rr] : "out of range";
    printf("  reset reason  : %d (%s)\n", rr, name);
    if (rr == 9) {
        printf("                  Brownout is a POWER fault -- bad cable, weak\n");
        printf("                  supply, or a hub that cannot source enough.\n");
    }
}
